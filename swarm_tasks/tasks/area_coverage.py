...
from swarm_tasks.utils.weight_functions import (
    linear_trunc,
    hyperbolic,
    sinusoidal,
    triangular,
...
NEIGHBOURHOOD_RADIUS = 10  # 4.5


def disp_exp_area_cvg(
    bot,
    ...
    if use_base_control:
        ...
        cmd += base_control.exp_obstacle_avoidance(bot, field_weights=obstacle_field_weights)
    return cmd
...
def goal_area_cvg(
    ...
    if True:
        ...
        cmd += base_control.obstacle_avoidance(i, bot, goal, field_weights=obstacle_field_weights)
        # cmd+=line(bot)
